...
%% API (used by mod_push_custom_keepalive).
...
delete_old_sessions(Days) ->
    ...
    DBTypes = lists:usort(
		lists:map(
		  fun(Host) ->
			  case mod_push_custom_opt:db_type(Host) of
			      sql -> {sql, Host};
			      Other -> {Other, global}
			  end
...
notify(LServer, PushLJID, Node, XData, Pkt, Dir, HandleResponse) ->
    ...
    PubSub = #pubsub_new{publish_new = #ps_publish_new{node = Node, items = [Item]},
...
make_summary(Host, #message{from = From} = Pkt, recv) ->
    case {mod_push_custom_opt:include_sender(Host),
	  mod_push_custom_opt:include_body(Host)} of
	{false, false} ->
	    undefined;
...
cache_opts(Opts) ->
    MaxSize = mod_push_custom_opt:cache_size(Opts),
    CacheMissed = mod_push_custom_opt:cache_missed(Opts),
    LifeTime = mod_push_custom_opt:cache_life_time(Opts),
...
use_cache(Mod, Host) ->
	...
	false -> mod_push_custom_opt:use_cache(Host)
    end.
