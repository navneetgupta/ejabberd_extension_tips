%%%----------------------------------------------------------------------
%%% File    : mod_push_custom_keepalive.erl
...
-module(mod_push_custom_keepalive).
...
start(Host, Opts) ->
    case mod_push_custom_keepalive_opt:wake_on_start(Opts) of
	true ->
	    wake_all(Host);
	false ->
	    ok
...
reload(Host, NewOpts, OldOpts) ->
    case {mod_push_custom_keepalive_opt:wake_on_start(NewOpts),
	  mod_push_custom_keepalive_opt:wake_on_start(OldOpts)} of
	{true, false} ->
	    wake_all(Host);
	_ ->
	    ok
...
c2s_stanza(#{push_enabled := true, mgmt_state := pending} = State,
	   Pkt, _SendResult) ->
    case mod_push_custom:is_incoming_chat_msg(Pkt) of
	true ->
...
c2s_session_pending(#{push_enabled := true, mgmt_queue := Queue} = State) ->
    case mod_stream_mgmt:queue_find(fun mod_push_custom:is_incoming_chat_msg/1,
				    Queue) of
	none ->
...
c2s_handle_cast(#{lserver := LServer} = State, push_enable) ->
    ResumeTimeout = mod_push_custom_keepalive_opt:resume_timeout(LServer),
    WakeOnTimeout = mod_push_custom_keepalive_opt:wake_on_timeout(LServer),
...
c2s_handle_info(#{push_enabled := true, mgmt_state := pending,
    ...
    mod_push_custom:notify(State, none, undefined),
    {stop, State};
